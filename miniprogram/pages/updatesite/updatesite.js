// pages/updatesite/updatesite.js
...
Page({
  ...
  onChangeName: function (event) {
    if (!(/^\w{2,}$/.test(event.detail.value))) {
      wx.showToast({
        title: '联系人最少2个字符',
        duration: 2000,
        icon: 'none',
  ...
  onLoad: function (options) {
    console.log(options);
    var i = JSON.parse(options.item);
    this.setData({
      item: i
    })
    this.setData({
      uname:this.data.item.uname,
      radio: this.data.item.usex,
      phone: this.data.item.uphone,
      address: this.data.item.uaddress,
      doorplate: this.data.item.udoorplate,
      uli: this.data.item.uli
    })
    console.log(this.data.item.uname);
  },
